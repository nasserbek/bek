...
void read_NVM_character(byte ch, const byte* addr, byte progmem)
{
  byte x;
  byte y;
  // disable display
  ...
  spi_writereg(VM0_reg,DISABLE_display);
  spi_writereg(CMAH_reg,ch);  // set start address high
  spi_writereg(CMM_reg,READ_nvr);
  for(x = 0; x < NVM_ram_size; x++) // write out 54 (out of 64) bytes of character to shadow ram
  {
    spi_writereg(CMAL_reg,x); // set start address low
    Serial.print("0x");
    y=spi_readreg(CMDO_reg);
    if(0x00==y)
    {
      Serial.print("00");
    }
...
void read_fontdata()
{
  ...
  for (int ch=0; ch<256; ch++) {
    //Serial.print((int)ch);
    read_NVM_character(ch,fontdata+64*ch,1);
    //Serial.println(" OK");
    delay(30);
  }
  // force soft reset on Max7456
