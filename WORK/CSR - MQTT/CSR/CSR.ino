...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  int Received_Json = resultS.toInt() ;    //Get the values of the received Json
...
void remoteControl(int cmd )
{
     if (blynkConnected)  myBlynk.blynkRCLed(1, cmd); 
       ...
       else if( RC_Remote_CSR2)
      {
        doc2["RC"] = cmd;
        serializeJson(doc2, Json); // print to client
        client.publish(AWS_IOT_PUBLISH_TOPIC_2, Json); 
      } 
       ...
       else if( RC_Remote_CSR3)
      {
        doc2["RC"] = cmd;
        serializeJson(doc2, Json); // print to client
        client.publish(AWS_IOT_PUBLISH_TOPIC_3, Json); 
      } 
