...
bool blynk::init() 
{
  ...
  if(_wifiIsConnected)
    {
      #ifdef LOCAL_SERVER
    //      Blynk.config(BLYNK_AUTH_TOKEN, IPAddress(192,168,1,77), 8080);
        #ifdef  WIFI_SSID_METEOR_FREE
            Blynk.begin(BLYNK_AUTH_TOKEN, WIFI_SSID_METEOR_FREE, WIFI_PASSWORD_METEOR, IPAddress(192,168,1,196), 8080);
        #endif

        #ifdef  WIFI_SSID_FREE 
            Blynk.begin(BLYNK_AUTH_TOKEN, WIFI_SSID_FREE , WIFI_PASSWORD, IPAddress(192,168,1,77), 8080);
        #endif    
        _blynkIsConnected =true; 
      #endif
