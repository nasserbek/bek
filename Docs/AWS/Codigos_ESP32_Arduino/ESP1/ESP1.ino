...
//Variables de botones, led y potenciómetro
float PotVal;
int BtnPin = 23;
int LedPin = 4;
int Btn = 0;
...
void setup() {
  ...
  Serial.begin(115200);
  //Configurar pines del botón y del led
...
void loop() {
  ...
  //Flanco de subida botón de led y botón WiFi
  ...
  //Se hace la lectura del valor del potenciómetro
  PotVal = Volts();

  // Las siguientes lineas se encargan de enviar cada 2 segundos
  //las el valor del potenciómetro y el estado del led
  ...
  {
    doc2["Pot"] = PotVal; //Guardar el valor del potenciómetro en formato Json
    ...
    mqttClient.publish("ESP1/Val",Json); //Se publica el Json serializado en el tópico ESP1/Val
...
//Función para leer el voltaje en el potenciómetro
float Volts(void)
{
  int adcVal = analogRead(PIN_POT); //Leer el pin analógico
  float Volt = adcVal * (3.3 / 4095); //Hacer la conversión a Volts
  return Volt; //Regresar el valor de voltaje
}
