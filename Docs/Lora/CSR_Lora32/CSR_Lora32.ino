...
void sendReadings() {
  ...
  display.drawString(display.getWidth() / 2, display.getHeight() / 2, "LoRa packet sent!"); 
  display.display();
  delay(2000);
  display.drawString(display.getWidth() / 2, display.getHeight() / 2 - 16, "Temperature:");
  display.display();
...
void loopLora()
{
    ...
    if (LoRa.parsePacket()) {
        ...
        while (LoRa.available()) {
            Lorarecv += (char)LoRa.read();
        }
        count++;
        display.clear();
        display.drawString(display.getWidth() / 2, display.getHeight() / 2, Lorarecv); 
        myBlynk.TerminalPrint(Lorarecv);
        String info = "[" + String(count) + "]" + "RSSI " + String(LoRa.packetRssi());
        display.drawString(display.getWidth() / 2, display.getHeight() / 2 - 16, info);
        myBlynk.TerminalPrint(info);
        display.display();
    }
#endif
}
...
#if defined CSR2  //4 Relays Active LOW and 2 I2C Controllers
  switch (receiver)
        {
          case 2:
          case 3:
