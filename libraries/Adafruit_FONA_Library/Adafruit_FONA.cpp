...
boolean Adafruit_FONA::sendSMS(char *smsaddr, char *smsmsg) {
  ...
  while (strcmp(replybuffer, "OK") != 0) {
	  readline(20000); // read buffer
	  DEBUG_PRINTLN(replybuffer);
      if (millis() - timeout > 30000L) return false;
  }
  ...
  DEBUG_PRINT("Line 3: "); DEBUG_PRINTLN(strlen(replybuffer));DEBUG_PRINTLN(replybuffer);
  if (strstr(replybuffer, ">") == 0) {
    return false;
  }
  readline(20000); // read >
...
boolean Adafruit_FONA::deleteSMS(uint8_t i) {
    if (! sendCheckReply(F("AT+CMGF=1"), ok_reply)) return false;
  // read an sms
  char sendbuff[12] = "AT+CMGD=1,4"; //"AT+CMGD=000";
 // sendbuff[8] = (i / 100) + '0';
 // i %= 100;
// sendbuff[9] = (i / 10) + '0';
//  i %= 10;
